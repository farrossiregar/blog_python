...
from .models import User, City, Country, Countrylanguage, Articles

@login_required
def home(request):
    
    result = {"cities": [], "country":[]}
    result['cities'] = City.objects.all().values()
    # result['country'] = Country.objects.all().values().order_by('code')[:20:1]
    result['country'] = Country.objects.filter(continent='Asia').values().order_by('code')[:20:1]
    result['headline1'] = Articles.objects.filter(livereport='1').values().order_by('id')[:1:1]
    result['headline23'] = Articles.objects.filter(livereport='1').values().order_by('id')[:2:1]
    result['latest'] = Articles.objects.all().values().order_by('id')[:25:1]
    
    # print(Cities)
    

    # email = "farros123@gmail.com"
    user = User.objects.all()
    # user = User.objects.filter(email=email)
    # user = User.objects.get(email=email)
    # return HttpResponse(Cities.values())
    
    # # print(user)
    return render(request, "home.html", result)


# def detail(request, url_alias):
def detail(request, id):
    result = {"detail":[]}
    # result = Articles.objects.filter(alias=url_alias).values().order_by('id')[:1:1]
    result['detail'] = Articles.objects.filter(id=id).values().order_by('id')[:1:1]
    
    return render(request, "detail.html", result)

    # result = Articles.objects.get(alias=url_alias)
...
def signup_validate(request):
    ...
    # return HttpResponse(body.get("email"))
